...
export class EmailQueueConsumer {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    const emailAddress = configService.get<string>('MAIL_FROM_ADDRESS') || 'noreply@terracore.local';
    const emailName = configService.get<string>('MAIL_FROM_NAME') || 'TerraCore';
    this.fromAddress = `${emailName} <${emailAddress}>`;
  }

  @EventPattern('send-email-verification')
  async handleEmailVerification(@Payload() payload: { userEmail: string; data: EmailVerificationData }, @Ctx() context: RmqContext) {
    const channel = context.getChannelRef();
    const originalMsg = context.getMessage();

    try {
      const verificationLink = `${this.appUrl}/verify-email?token=${payload.data.verificationToken}`;
      const html = EmailVerificationTemplate.generate(payload.data, verificationLink);
      ...
      this.logger.error(`❌ Failed to send verification email to ${payload.userEmail}: ${error.message}`);
      channel.nack(originalMsg, false, false);
    }
  }

  @EventPattern('send-welcome-email')
  async handleWelcomeEmail(@Payload() payload: { userEmail: string; data: WelcomeEmailData }, @Ctx() context: RmqContext) {
    const channel = context.getChannelRef();
    const originalMsg = context.getMessage();
    ...
    try {
      ...
      this.logger.error(`❌ Failed to send welcome email to ${payload.userEmail}: ${error.message}`);
      channel.nack(originalMsg, false, false);
    }
  }
}