...
export async function parseGeometryFile(
  ...
  if (ext === '.shp') {
    try {
      ...
      return extractAndCombineGeometries(
        geojson,
        PARCEL_MESSAGES.SHAPEFILE_EMPTY,
      );
    } catch (error) {
      ...
      throw new BadRequestException(
        `${PARCEL_MESSAGES.GEOMETRY_PARSE_ERROR}: ${details}`,
      );
    }
  }
...
function parseKmlString(kmlContent: string): Geometry {
  ...
  if (placemarks.length === 0) {
    ...
    try {
      ...
      const errorMsg =
        toGeoJsonError instanceof Error
          ? toGeoJsonError.message
          : String(toGeoJsonError);
      throw new BadRequestException(
  ...
  for (let i = 0; i < placemarks.length; i++) {
    ...
    if (hasPolygon) {
      if (coordPairs.length >= 4) {
        ...
        console.warn(
          `Polygon "${name}" has too few coordinates (${coordPairs.length})`,
        );
        continue;
      }
...
function extractAndCombineGeometries(
  ...
  if ((data as FeatureCollection).type === 'FeatureCollection') {
    ...
    console.log(
      `Combining ${validFeatures.length} features into a single parcel geometry`,
    );
...
function combineGeometries(geometries: Geometry[]): Geometry {
  ...
  if (types.size === 1 && types.has('LineString')) {
    const coordinates = geometries.map((g: any) => g.coordinates);
    console.log(
      `Creating MultiLineString from ${geometries.length} LineStrings`,
    );
    return {
  ...
  console.log(
    `Creating GeometryCollection from ${geometries.length} mixed geometries`,
  );
  return {
...
type ShapefileParser = (
  input: ArrayBuffer | Buffer | Blob | string,
) => Promise<any>;
    ...
    shapefileParserPromise = (async () => {
      try {
        ...
        if (typeof shpjs === 'function') {
          parser = shpjs;
        } else if (typeof shpjs.default === 'function') {
          parser = shpjs.default;
        } else if (typeof shpjs.parseZip === 'function') {
          parser = shpjs.parseZip;
        } else {
          const functionExport = Object.values(shpjs).find(
        ...
        if (!parser || typeof parser !== 'function') {
          throw new Error(
            'Could not find valid parser function in shpjs module',
          );
        }
