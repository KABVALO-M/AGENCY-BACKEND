...
export class ParcelsService {
  ...
  async create(
    ...
    const parcelData: DeepPartial<Parcel> = {
      ...
      imageUrls,
      shapefileUrl: shapefile
        ? `/uploads/parcels/${shapefile.filename}`
        : undefined,
    };
  ...
  private scheduleGeoServerSync() {
    setImmediate(() =>
      this.geoServerSyncService
        .syncAll()
        .catch((error: Error) =>
          this.logger.error(
            `GeoServer sync failed after parcel creation: ${error.message}`,
            error.stack,
          ),
        ),
    );
  ...
  async addClimateMetric(
    parcelId: string,
    dto: CreateParcelClimateMetricDto,
    user: AuthenticatedUser,
  ) {
    const parcel = await this.getParcelOrThrow(parcelId);
    const auditUser = this.auditUser(user);
  ...
  async upsertRiskInput(
    ...
    if (!existing) {
      ...
      existing.normalizedScore =
        dto.normalizedScore ?? existing.normalizedScore;
      ...
      existing.lastEvaluatedAt =
        dto.lastEvaluatedAt ?? existing.lastEvaluatedAt;
  ...
  async refreshMaterializedViews(dto: RefreshMaterializedViewDto) {
    ...
    return {
      message: PARCEL_MESSAGES.MATERIALIZED_VIEW_REFRESHED,
      views: targets,
    };
  }
  ...
  async getMaterializedViewStatuses() {
    ...
    return {
      message: PARCEL_MESSAGES.MATERIALIZED_VIEW_STATUS_FETCHED,
      data: statuses,
    };
  }
